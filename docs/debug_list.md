...
- 2026-09-25：完成通知引用了会话目录之外的本地 PDF/ZIP 时，卡片只显示路径，没有「查看」按钮。引用解析、通知脚本和回复绑定都拒绝绝对路径，回调又把路径拼回会话目录，目录外文件和二进制文件都打不开。现在会话位于用户主目录内时，主目录下真实、非符号链接、非隐藏、非凭证文件可以生成查看按钮；会话不在主目录内时，只信任会话目录的父目录。回调按绑定中的绝对路径读取，不拼回会话目录；不能按 UTF-8 预览的文件走原有下载。伪造的引用序号、文件系统根、主目录本身、符号链接和敏感文件名仍然拒绝。会话目录内的相对路径引用保持不变。

- 2026-09-24：SSH 配置使用 `Host 10.64.35.29 moe-remote-35-29` 定义地址和别名时，看板最初把两个名称合成一项；拆分后又将同一服务器显示两次。根因是看板把 SSH 的多个可连接名称当成多个服务器预设。现每个 `Host` 指令只取第一个非通配符名称供看板显示，SSH 的两个名称仍都可连接；回归测试覆盖多名称与通配符。运行中的 `/api/ssh-hosts` 只返回一个该服务器条目。

- 2026-09-23：快捷回复选中模板后还要点击「预览 / 编辑后发送」，且另外发出一张编辑卡片，和用户希望的单卡预览流程不符。现在目标和模板使用同一消息卡片的下拉回调更新，选中模板原位显示可编辑正文；旧版直发按钮不再投递，只有编辑表单明确提交才进入 Codex 队列。

- 2026-09-23：快捷回复编辑卡片最初把单个输入框上限设为 8000 字，超过飞书 Card 2.0 的 1000 字限制。现将长模板按 Unicode 边界分段编辑、按序拼接，服务端校验每段长度与完整字段；回归覆盖多行、emoji、缺段和伪造超长输入，避免截断或卡片渲染失败。

- 2026-09-23：Claude 完成通知解析出了最终回复，但漏掉触发本轮的用户问题。`message.content` 为纯字符串时，解析器只生成记录项，没有写入完成关联使用的文本缓存；现在统一累积纯文本和文本块，并由回归测试覆盖带工具调用的完整 turn。
- 2026-09-23：分组排列下窗格标题拖不动，只能在自由排列里交换位置。分组模式现在同样允许拖动标题，放下即交换组内显示顺序并随当前页面保存；分组外的会话拖入不生效，当前输入也不变。切换到某个分组时，当前页签自动改成该分组的名字，重名时追加序号。
- 2026-09-23：聚焦页打开到两页以上时，「默认」「页面 1」「页面 2」页签叠在同一个位置，只能看见最上面一个。页签按钮被误写进设置气泡的绝对定位规则，脱离了页签行的排列。现页签按钮回到正常排列，气泡仍锚定在页签下方；端到端测试校验三个页签横向铺开、再点当前页弹出设置、Esc 关闭。
- 2026-09-23：页签从「默认」开始，点「＋」才出现「页面 1」。再点当前页签会在页签下方弹出小气泡，就地改名、切换排列和屏幕布局、关闭页面，不必打开左侧「屏幕布局」菜单；只剩最后一页时不能关闭。
- 2026-09-23：在「默认」和「页面 2」之间切换时，标签会把当前页挪到最前，两页看起来像互换了位置。标签行现在固定按创建顺序渲染，当前页只改变高亮，页面顺序和各自记住的布局都不变。

- 2026-09-23：聚焦页粘贴图片只认 Codex，Claude 会话会提示“不是可识别的 Codex 会话”。Claude 没有 queue 附件命令，现在按 tmux pane 的 `--resume`/`--session-id` 或工作目录唯一的 `~/.claude/projects` 记录定位会话，再用 `claude --resume --input-format stream-json` 把图片 base64 投进原会话；定位不唯一时拒绝发送，临时图片无论成败都立即删除。

- 2026-09-22：Grok/Codex 输入框每个字符都会把整份会话快照推到前端，聚焦页随之整树重绘，打字仍然发黏。普通按键现在与终端输出合并广播状态（默认最多约每秒一次），前端对内容相同的会话记录保持原引用，并稳定终端就绪回调，避免快照抖动让 xterm 无效重绘；任务摘要、Git、标签和连接信息等真实变化仍完整更新。

- 2026-09-22：看板会话里打字要过几秒才进输入框。根因是每个按键都会等待 tmux attach（最多 1 秒），普通输入还要等待终端协议探测回复（最多 250ms）；连续按键会把等待叠成数秒。现 attach 探测在后台复用，未附着时普通输入立即走受约束的 tmux pane 写入，附着后直接走 PTY；协议回复仍按独立通道优先处理。回归覆盖探测未完成时连续输入不被阻塞、PTY 普通输入不再等待能力回复，以及替换运行时不会误用旧就绪状态。

- 2026-09-22：单窗格切换 session 时，旧 xterm 虽保留在缓存数组中，但把当前会话移到数组首位会重排 React/Suspense 子树，导致 WebSocket 关闭、xterm 重建并再次回放历史，快速切回仍明显卡顿。现缓存层保持稳定挂载顺序，当前与最近会话短时保温 8 秒；往返切换复用同一 xterm 和连接，超时后再释放隐藏连接。浏览器回归验证两次切换只各建一条连接、实例身份不变且到期正常关闭。

- 2026-09-22：首次切到历史较重的终端时，单个大 replay 写入会让 xterm 连续占用主线程，窗格切换和其他输入事件要等解析间隙。现保持完整 replay 内容与顺序，把初始历史拆成小块并按有界批次让出事件循环；实时输出在同一有序队列中等待历史写完，避免提速时把新输出插到旧历史中。局域网真实 512 KiB 回放抽样中，目标窗格切换呈现从秒级等待降到约 0.6 秒，切回保温会话约 0.5 秒。

- 2026-09-22：34 张 tmux 卡片、29 个在线 PTY 的运行环境中，后端长期占用约 86% CPU，健康检查最高延迟超过 3 秒。CPU profile 显示完成通知定位每次都为每个 pane 重新读取并 UTF-8 解码全部 Codex 历史 JSONL 的 64 KiB 头，即使进程命令行已经带有明确 `codex resume <thread-id>`。现优先按明确 thread ID 精确读取目标 rollout，并按文件 `ctime + mtime + size` 缓存其余元数据；文件追加或替换会自动失效。相同运行态重载后 CPU 五秒抽样均值约 15%（1–29%），后端健康检查多数约 1–2 ms、最高约 20 ms。

- 2026-09-22：电脑端聚焦分屏和顶栏留白偏大，多窗时终端面积被 Logo 行、主内容 18px 内边距、8px 网格缝和 34px 会话切换条挤占。现收紧顶栏、主内容外框和分屏边角；小窗标题栏压到约 22px，「当前输入」只保留徽章并隐藏重复的「输入中」按钮。回归覆盖 CSS 密度断言和活动窗格徽章结构。收紧后 Logo 被压成约 16px 高的两行字标，和旁边 15px 的「电脑端 Coding Kanban」不齐；现按 24px 高度对齐标题行，标题改为 14px / 24px line-height。
...
- 2026-09-18：飞书完成卡片底部「查看完整记录」点击后没有反应。根因有两处：`lark-cli` 回传的 `action_value` 可能已经是对象，原先只 `JSON.parse` 字符串会得到空动作并静默丢弃；同时该入口与文件工作区共用 `replyEnabled`，默认关闭回复控制时回调直接 `ignored_disabled` 且不提示。现兼容对象/字符串回调，把完成卡片记录查看、分页和导出拆成只读能力（飞书通知开启即可），绑定失效或会话不可用时发送说明；文件工作区和向 Codex 发指令仍受回复控制保护。回归覆盖对象回调、回复控制关闭时仍能打开记录，以及菜单打开完整工作区继续拒绝。
...
- 2026-09-17：其他服务器上的 tmux Codex 在“完整记录”中可能直接显示无记录，或因读取远端历史超时而长期空白。根因是远端卡片保留了首次接入时的 `agentKind` 和工作目录，切换 tmux 窗格后仍按旧目录匹配；同时 SFTP 会串行遍历全部日期目录，再一次读取所有 JSONL 头。现在打开记录时先探测远端 tmux 当前活动窗格，仅对正在运行 Codex 的 `node/codex` 窗格读取其实时工作目录；SFTP 目录按最多 8 路并发遍历，记录头按 8 条一批从新到旧查找并在首个匹配后停止。真实远端 `dvs-0` 的 701 份历史已从超时恢复为约 7 秒返回精确会话，回归覆盖旧卡片跟随活动窗格、批量提前停止和递归读取并发。

- 2026-09-17：文件系统图片预览复用了文本/二进制分段接口，默认只读取前 `64 KiB`、最多 `256 KiB`；较大的 JPEG 等图片会被浏览器按不完整数据解码，因此只能看到上半部分。现在桌面与手机端统一通过 `POST /api/fs/image` 流式读取完整图片并用可释放的 Blob URL 展示，预览容器按比例完整缩放；接口校验图片 MIME、拒绝目录与非图片、单张限制 16 MiB，并同时支持本机和 SSH/SFTP。回归覆盖超过旧上限的完整响应、远端读取、类型/大小拒绝以及两端不再拼接截断 Base64。

- 2026-09-17：远程 `research_26` 的 tmux 会话虽然开启了 `mouse on`，但主机的 `terminal-features` 缺少 `mouse`，浏览器 xterm 因此没有进入鼠标报告模式，点击看起来无响应。现 tmux 新建、附着和重连命令补齐 `xterm*:mouse` 与会话级 `mouse on`；前端对已登记 tmux 终端在回放完成后主动启用 SGR 鼠标报告，兼容已有远程会话和不完整的 tmux 能力协商。回归覆盖远程 tmux 无鼠标握手时仍发送点击报告。

- 2026-09-17：`research` 的终端程序可能在启动后再次关闭浏览器 xterm 的 mouse tracking，导致滚轮看起来失效。现滚轮路由发现登记为 tmux 的交互终端处于 `none`/`x10` 状态时，会先恢复 SGR mouse mode，并直接补发当前带坐标的 `64/65` 滚轮报告，避免恢复动作吞掉第一下滚轮；Shift+滚轮仍只浏览本地 xterm 历史。回归覆盖终端主动关闭 mouse mode 后滚轮仍到达 tmux。

- 2026-09-16：桌面聚焦页切换到左右分屏后，第二个窗格可能整块黑屏。根因是自动补位优先选中了已经离线且没有 PTY 回放的历史 tmux 会话，前端仍为它挂载 xterm；同时缓存终端从隐藏窗格重新显示时缺少一次确定性的 fit。现自动排布优先使用在线/降级会话，离线会话改为带恢复提示的轻量预览，隐藏终端在重新可见和进入视口时强制 fit/refresh。回归覆盖离线窗格不建立 WebSocket、可用会话优先补位和分屏后的实际回放。

- 2026-09-15：飞书机器人“任务总览”在回复控制关闭时也完全没有响应，导致无法查看当前 session。根因是菜单与 `im.message.receive_v1` 共用 `replyEnabled` 监听门控；现将任务总览拆为只读能力，飞书通知配置开启即可查看状态、摘要、翻页和刷新，发送指令、通知回复、完整记录及文件操作仍由回复控制开关保护。回归覆盖默认关闭回复控制时的总览卡片和事件消费者。

- 2026-09-15：只读总览代码已恢复，但机器人菜单仍可能无响应。运行态定位到飞书应用尚未订阅 `application.bot.menu_v6`，同时 `card.action.trigger` 被其他项目 `work_hub` 的消费者占用；后端此前只退避不暴露 CLI 原因。现记录有界启动诊断并在配置文档说明事件发布、单消费者冲突和独立 profile 处理方式；不擅自停止其他项目消费者。

- 2026-09-13：手机端点击输入框弹出软键盘后，页面仍保留底部主导航的 `88px` 占位和安全区内边距，浏览器平移可视视口时页面顶部也没有同步偏移，导致终端内容上跳、快捷键及发送/粘贴区与键盘之间出现大块空白。现在快捷键和输入区组成不可压缩的连续控制区；输入框聚焦时隐藏底部主导航、清除其占位和多余安全区，并同步 `VisualViewport` 的高度与顶部偏移。回归测试覆盖键盘感知 viewport 声明、视口偏移归一化和聚焦布局规则。

- 2026-09-10：部分长任务的飞书完成卡片已有「查看完整记录」按钮，却没有「你的问题」。完整记录按钮依赖精确 Codex thread 绑定，而问题提取器只扫描 `task_complete` 前最后 1 MiB；真实长任务的问题距离完成事件约 3 MiB，因此被安全省略。现在保留 1 MiB 快速探测，仅在完成存在但问题缺失时扩大到最多 8 MiB 做一次有界恢复，本机与 SSH 使用相同规则；仍无法可靠关联时继续省略而不猜测。

- 2026-09-08：运行中的 Codex 在工作目录被重命名/删除后，完整记录误报未找到。定位器此前强制比较进程当前目录与记录启动目录；现在对已确认 Codex 进程持有的主会话文件优先使用进程身份，其他读取进程和目录兜底仍校验目录，继续排除子代理。回归覆盖删除目录、非 Codex 读取者、显式 resume 与同目录多终端隔离。

- 2026-09-07：完整记录折叠按钮的独立底栏和最小高度拉长每条消息；最终改为顶部时间同行的小字按钮，移除底栏及额外留白，扩展透明点击区域并保留键盘焦点。浏览器回归覆盖按钮高度、时间同行及展开/折叠。

本文档根据现有仓库记忆整理历史 bug 修复记录。后续每次修复 bug，都应在本文件追加简短记录，说明现象、根因和关键修复点。

- 2026-09-07：飞书卡片的普通 Markdown 已渲染，但 LaTeX 公式仍显示源码。卡片 Markdown 不等同于网页的 KaTeX 数学扩展；发送端新增本地公式排版、图片上传和嵌入，代码示例不转换，异常或环境不满足时保留源码继续通知。测试覆盖四种定界符、金额/代码保护、隐私裁剪先于渲染、图片分片完整性及失败回退。

- 2026-09-07：飞书完成卡片正文显示 Markdown 源码，原因是正文组件使用 plain_text。改为 Card 2.0 markdown，标题和项目元数据仍用纯文本；长正文优先按行分片并补齐跨片代码围栏，转义飞书人员标签。红绿灯测试覆盖富文本组件、完整内容保留、跨片代码块及人员标签。

- 2026-09-07：完整记录全屏后，鼠标位于底层终端范围内时滚轮无法滚动记录。终端的 document 级滚轮捕获遗漏全屏记录浮层，按坐标拦截了滚轮默认行为。将 `.agent-transcript-fullscreen-backdrop` 加入终端滚轮避让列表；单测及真实浏览器红绿灯验证全屏记录上下滚动、Esc 返回侧栏。

- 2026-09-07：飞书回复偶发未提交，尤其 `/goal` 回复没有形成 Codex 用户消息，但已被标记 processed。旧链路仅确认 PTY 写入及固定 50ms 后的 Enter，无法确认 TUI 实际接受。改为复用 Codex 原生文本 queue，按活动会话 ID 提交完整正文，队列成功才去重；失败不自动补发键盘输入，日志记录处理结果。测试覆盖原生队列确认、忙碌会话、完整 `/goal` 正文、无活动会话、重复事件、SSH 参数及失败不标记。
...
- 飞书完成通知有时能收到、有时漏掉，尤其是 Codex 回复后 15 秒内继续发下一条消息时稳定漏报。根因是通知器只依赖终端 `running → idle` 边沿，而空闲检测需要连续静默 15 秒，快速连续的多个 Codex turn 会被合并成同一个运行区间。修复为从本机或 SSH Codex JSONL 尾部读取原生 `task_complete`，按 `turn_id` 建立基线、逐 turn 触发并去重；终端状态边沿仅保留为非 Codex 或结构化记录不可用时的降级，后续 idle 不会重复发送。探测按输出变化防抖，并缓存 session 文件与短期定位结果，避免恢复到高频全量扫描。红绿灯测试覆盖终端始终 running 的连续两次完成、旧记录不补发、idle 不双发、本机/远端完成记录解析和完整正文保留。
- 飞书完成通知只显示类似终端标题的截断“摘要”，无法看到 Codex 对话真正的最终答复。根因是通知器直接读取卡片的 `lastAgentMessageSummary/outputPreview`，而该字段为 180 字符摘要，脚本又把正文限制为最多 600 字符。修复为完成边沿复用 tmux 活动 pane 与 Codex transcript 定位，读取最后一条完整 assistant 消息；发送脚本保留换行和缩进，长正文分片且每片独立幂等，只有结构化记录不可用时才回退摘要。红绿灯测试覆盖 shell 标记的 tmux Codex、完整正文替换、分片重组和多片幂等发送。
- “设置 → 飞书通知”已经开启，但按钮开启前启动且没有加载用户级 `notify` 的 Codex 完成后不发送。根因是旧开关只控制原生 hook 被调用后的发送逻辑，无法向运行中的 Codex 动态注入 hook。修复为 Kanban 后端持续订阅全部已登记会话，只对新的 `running → idle/exited/detached` 完成边沿发送；启动时保留旧开关值并迁移到 `kanban` 接管模式，旧原生 hook 自动静默以避免重复。恢复门控同时区分重启前确实运行中的会话和恢复期间短暂变成 `running` 的旧空闲会话，避免重启批量误报。红绿灯测试覆盖已运行会话、关闭/重开、初始空闲不补发、恢复门控、重复快照去重和固定无 shell 发送命令。
- 桌面窗口未最大化时，顶部菜单或通知层可能覆盖“完整记录”的刷新、关闭按钮。根因是记录弹层仍挂载在聚焦视图内部，受祖先层叠上下文约束，且只使用比例高度，没有按窗口实际动态高度保留安全边距。修复为把完整记录通过 Portal 挂载到 `document.body` 的独立模态层，层级高于应用菜单和通知、低于专用全屏 Diff；弹层按 `dvh` 和自适应边距限制最大高度，标题操作区固定不收缩，窄高桌面窗口中按钮仍保持可见可点。
- Markdown 文件预览中的本地或 SSH 图片会持续闪烁，服务端同时约每秒收到一次相同图片请求。根因是会话快照刷新会重建内容相同但引用不同的资源上下文，`ReactMarkdown` 每次又收到一组全新的自定义组件函数，导致图片组件反复卸载、释放 Blob URL、重新读取并挂载。修复为按文档路径、项目根及 SSH 字段比较资源上下文，并对 Markdown 渲染器使用该语义比较；自定义链接、标题和图片组件表按实际资源字段稳定缓存，普通看板刷新或正文更新不再换掉已挂载图片。
- 电脑端六屏等多终端布局在浏览器宽度降到 `1180px` 时就过早从三列两排变成单列多行，即使主终端区仍有足够横向空间。根因是布局按整个视口的固定媒体查询判断，没有考虑侧栏和主终端的真实可用宽度。修复为在主终端容器上启用命名容器查询，三列/四列布局保持到终端区实际小于 `720px` 才切换单列；侧栏展开或收起也会按真实剩余空间响应。
- Markdown 文件预览中的 `![说明](./images/a.png)` 只显示破图，手机端与电脑端一致。根因是共享 Markdown 渲染器把相对 `src` 原样交给浏览器，浏览器会相对看板页面 URL 请求，而不是相对当前文档所在的本地/SFTP 路径。修复为渲染器接收文档、项目根和 SSH 上下文，图片接近视口时经专用接口流式读取并显示 Blob；服务端支持文档相对、项目根相对和根内绝对路径，拒绝路径/符号链接越界、非图片及超过 16 MiB 的资源，前端失败保留重试入口并在卸载时释放对象 URL。外部 HTTP(S) 图片保持原生懒加载。
- 电脑端 Markdown 全高预览缺少标题目录，实时分屏的源码与渲染区也各自滚动，长文档难以定位同一章节；同时桌面端只拿预览前缀却仍允许保存，存在截断覆盖大文件的风险。现为渲染标题生成稳定行锚点和可折叠目录，分屏增加默认开启、可关闭的双向比例同步；桌面端连续读取最多约 1 MiB 为一个滑动窗口，超大文件仅保留当前段并提供前后导航，未完整载入时强制只读，手机端不变。
- 电脑端文件系统双击文本或 Markdown 会打开覆盖式浏览/编辑窗口，导致右侧终端被遮挡，无法边看文件边向 Agent 指出修改位置。修复为复用文件侧栏现有预览组件：双击后在左侧面板内进入全高专注预览，提供“返回文件列表”，普通文本编辑和 Markdown 的渲染/源码/分屏均在原面板完成；删除重复的 Markdown 弹窗组件和第二份渲染链，手机文件系统保持不变。
- 电脑端在聚焦页切换终端卡片时会出现约数百毫秒卡顿，分组排列会随组内会话数同时挂载真实 xterm。根因包括：切换未上屏会话会销毁旧终端并重新回放最多 4 MiB 历史；分组离屏窗格也保持 WebSocket 和 xterm；服务端把数十万个 PTY 小片段保存在数组中，回放时反复 `join()`；focus POST 又返回完整会话快照触发二次全局渲染。修复为单屏保留当前及最近两个终端实例、分组仅为当前/可视附近窗格挂载真实终端、TerminalView 跳过未变化 props 的渲染；服务端 scrollback 按 64 KiB/128 片压缩并缓存物化结果，以逻辑头游标替代逐项 `shift()`，UTF-8 安全地保留完整配置窗口；focus 改为有序的 `204` 轻量确认并继续通过 WebSocket delta 同步状态。
- 新版 tmux 的 copy-mode 可能发送省略选择目标的 OSC52（`OSC 52 ; ; data`），前端过去只接受显式 `c` 目标，导致选中内容已经到达浏览器却没有写入剪贴板。解码器现同时接受标准的空目标和 `c` 当前剪贴板目标，其他选择目标、查询和非法 Base64 仍拒绝。
- 电脑端在看板终端内右击不再出现浏览器原生的复制、粘贴菜单。根因是活动 xterm 新增了捕获阶段 `contextmenu` 拦截并无条件 `preventDefault()`，以便 tmux 接管右键鼠标事件，却破坏了桌面端既有右键操作。修复为删除终端内容区的右键拦截及专用判断函数；终端标题栏的看板菜单、左键拖选和 tmux copy-mode 逻辑保持不变。
- “完整记录”弹窗首次打开时虽然先滚到底部，但底部附近的 Markdown 随后才懒加载并扩展真实高度，原来的一次性滚动位置不会跟随，导致视口停在较早对话，看起来自动向上翻了一段。修复为在初始懒渲染期间用 `ResizeObserver` 观察记录内容高度并持续锚定最新一条；用户主动滚轮、触摸或点击记录区时立即解除锚定，加载更早记录继续沿用原有视口恢复逻辑。
- 运行旧版 `tsx watch src/index.ts` 的看板拉取新代码后，后端约每 1.5 秒重启一次，前端持续显示“重连中”。根因是新启动路径无条件编译 `packages/shared`，编译改写 `dist` 后又被旧 watcher 当作依赖变化触发重启，形成“启动 → 编译 → 改写产物 → 重启”的闭环。修复为启动时逐个比较 shared 源码、配置与对应 JavaScript 产物的修改时间：仅缺失或过期时构建，产物已最新时直接启动；这样即使拉取前遗留的旧 watcher 仍在运行，首次构建后也会稳定收敛，不再自激重启。
- 手机端文件系统点击“新建”后没有输入框，也无法唤起软键盘。根因是创建流程依赖 `window.prompt()`，部分手机浏览器、PWA 或内嵌 WebView 会拦截系统提示框。修复为在底部菜单内直接渲染受控原生输入框，并在用户点击“新建”的同一手势中同步挂载、聚焦；输入字号保持 16px，避免 iOS 聚焦时自动缩放。重命名流程保持原有行为。
- 手机端即使只查看一个终端，任意会话输出也会让 `/ws/agent-sessions` 约每秒传输一次全部会话，实测多会话时持续占用明显流量并引发 JSON 解析与 React 更新。现改为每条连接首次全量、后续增量：只发送变化会话、删除 ID、焦点和时间戳，重连后重新建立全量基线。
- 手机进入或切换终端时会重复下载最多 4 MiB scrollback，造成明显流量突增。现由手机终端请求最近 256 KiB，后端严格校验请求值且不允许超过全局历史上限，并在 UTF-8 字符边界截取尾部；桌面回放行为不变。
- 手机文件系统只能点击进入或预览，无法执行桌面右键菜单中的常用操作。现为文件条目增加 600ms 长按底部菜单，滑动时自动取消，支持进入/预览、下载、重命名、删除和复制路径；删除仍需二次确认，操作失败会保留菜单并展示错误。
- “完整记录”的“继续加载”此前只限制前端挂载的 DOM，后端仍会 `readFileSync` 整份 Codex JSONL、解析全部条目并传给浏览器，数百 MiB 会话会造成后端与浏览器内存峰值。修复为服务端从文件尾部按 64 KiB 块和字节游标读取，每页只返回约 30 条且跨页保持工具调用/输出配对和 `exec` 过滤；轻量模式前端最多保留连续 90 条、完整模式最多 300 条，关闭弹窗或刷新后释放旧窗口。红绿灯测试覆盖 30/30/15 连续分页、超过单个读取块的消息、跨页 `exec` 过滤、HTTP 游标传递和轻量窗口淘汰。
- 电脑端聚焦页的会话标题、完整记录、变更、状态、布局和返回操作之间使用 12px 间距及较宽内边距，标题栏还允许换行，主窗口稍窄时会占据两行并压缩终端空间。修复为标题栏固定单行且整体收紧，会话名成为唯一可收缩项并以省略号显示，操作按钮保持完整文字和稳定点击区域；折叠态同步使用更紧凑的间距。
- 手机端“当前会话”下拉菜单覆盖终端后，鼠标滚轮会滚动后台 xterm 而不是菜单列表。根因是终端的 document 捕获阶段滚轮兜底按坐标接管事件，而移动端会话菜单没有被纳入弹层阻断目标。修复为将 `.mobile-session-picker-menu` 加入统一滚轮阻断列表，让菜单保留原生纵向滚动并继续使用 `overscroll-behavior: contain` 阻断滚动链穿透。
- 桌面聚焦视图的终端会话切换器虽然按分组展示，但组标题是静态文本且组内会话始终渲染，终端较多时无法收起暂时不用的分组。修复为把组标题改成带展开状态的可键盘按钮，复用现有分组折叠持久化并使用独立的 `terminal-switcher` 作用域，避免联动主页或侧栏；搜索时匹配分组保持展开，确保结果不会被隐藏。
- 手机端快捷键栏从方向键上开始横滑时会卡顿并可能误输入方向键，长按约 360ms 就连续发送也容易误操作。根因是重复键使用 `touch-action: none`，按下立即 `preventDefault` 并启动输入。修复为重复键允许原生横向滚动，移动超过 10px 即取消按键；单击在松手后只发送一次，静止按住满 3 秒才开始串行重复，同时将 `Enter / Tab` 移到方向键之前。
- 变更面板曾展示“引用文件”，但该入口只复制或插入 `@路径` 纯文本，没有文件内容注入或后端解析语义，并与“复制路径”重复。现已从普通和全屏 Diff 中移除该入口，同时删除手机端引用草稿的死状态，只保留行为明确的“复制路径”。
- 手机端默认先进入看板，用户每次都要再点“当前会话”才能操作终端；同时快捷键高频方向键位置靠后且不能长按，输入发送失败还会因为父层吞掉异常而清空草稿，“发送”和“粘贴执行”实际行为重复。修复为默认直接进入当前会话，全部快捷键仍保持单行常驻但按使用频率重排，方向键和退格使用等待式长按重复；输入区精简为安全 bracketed paste 的“发送 / 粘贴”，失败保留草稿并按未完成帧重试。
- 手机端“当前会话”下拉列表复用了看板按注意力和最近活动时间排序的结果，实时输出会让正在执行的会话不断换位，难以稳定选择。修复为给会话选择器建立独立排序：可继续优先、执行中其次、其它状态随后，各组按会话名称的中文拼音或字母顺序排列，并用会话 ID 打破同名并列；手机看板原有的需响应/待验收优先级保持不变。
- “本次任务”变更无法映射当前 Codex diff，尤其是本地 tmux 卡片被识别为 `node`/`shell` 时直接显示不可用。根因是接口只放行 `agentKind=codex`，解析器也只识别旧版 `custom_tool_call/apply_patch`，没有读取新版 Codex 按 `turn_id` 输出的 `item_completed/FileChange`。修复为复用 tmux 进程定位得到精确 session，按最新任务轮次直接映射原生新增、删除、修改和重命名记录，并保留旧版 `apply_patch` 兼容兜底；服务和路由回归测试分别覆盖轮次隔离与 tmux 卡片接入。
...
- 手机端变更面板直接展开全部文件，文件较多时会挤压实际 Diff 内容。紧凑模式改为单个下拉框选择文件，并为当前文件增加全屏 Diff 入口；全屏视图独立滚动，支持 Esc/按钮退出和复制路径。
...
- 多屏聚焦视图关闭文件/VS Code/变更侧栏后，从外部切到 `coding` 等会话，键盘仍可能持续输入旧的“清理空间”窗格。现场记录显示全局 focus 已切换后，退格、中文和快捷键仍写入旧 pane；根因是“监控窗格本地切换不请求 `/focus`”的优化把真正的外部 `focusedSession` 变化也排除在输入窗格同步之外。修复为区分两种变化：多屏内单击仍只更新本地 `activeSlotId`，不刷新全局状态；外部 focused session 改变时始终激活其可见窗格并转移唯一输入权。回归测试覆盖外部变化必须同步、同一 focused session 下本地切换保持解耦。
...
### 同目录多个 Codex 进程的完整记录选错会话

- **现象**: 选中 `xh3_refmodel` tmux 窗格中的 Codex 后点击“完整记录”，显示成同一工作目录下另一个更晚启动的 Codex 对话。
- **根因**: 当前 pane 的 Codex 进程可能在两次写入之间关闭 rollout JSONL 文件句柄；旧定位逻辑在没有打开文件时按工作目录选择最新记录，多个同目录进程无法区分。
...
- **根因**: 活动标题的 session switcher 使用了不同的最小高度，且 ResizeObserver 未判断容器尺寸是否真的变化。
- **修复**: 统一标题控件高度，仅保留选中态颜色、边框和字重；ResizeObserver 仅在容器宽高实际变化时执行 fit。
- **测试**: `tests/e2e/terminal-preview.spec.ts` 的双屏切换用例断言两侧 xterm 实例、WebSocket 和 refresh 计数均保持不变。

### 终端首次回放期间 Codex 握手或快速输入丢失

- **现象**: 页面打开前已启动的 Codex/Copilot TUI 卡在能力握手；聚焦页刚打开时快速输入也可能没有进入 PTY。
- **根因**: 回放过滤掉了已发生的 DA/DSR 查询，且完整 xterm 异步挂载期间键盘事件落到 `body`，没有终端 textarea 接收。
- **修复**: 服务端在浏览器未及时回复时使用保守的 VT 能力回复兜底；前端保持 xterm 协议响应通道开启，仅在回放完成前阻断普通输入，并暂存挂载窗口内的按键后按顺序重放。
- **测试**: `PtyRuntimeManager` 覆盖无浏览器挂载时的握手兜底；输入转发单测覆盖回放边界；Playwright Copilot 聚焦、字体调整、点击外部控件后输入用例全部通过。

### 多窗口布局部分终端加载缓慢或长期黑屏

- **现象**: 六屏、八屏或大分组布局一次打开多个终端时，部分窗格长时间保持纯黑，偶尔反复断线重试后才出现内容。
- **根因**: 多个 xterm 同时解析最多 4 MiB 的完整历史会阻塞浏览器主线程，连 WebSocket `open` 事件也不能及时派发；3 秒握手保护因此会误杀实际已在排队的连接，并形成同频重试风暴。
- **修复**: 聚焦页将首次完整回放限制为最多两个并发并优先当前输入窗格，直到 xterm 真正解析完回放才放行下一个；排队和解析期间继续展示轻量终端内容及加载状态，12 秒安全阈值防止异常窗格饿死队列。桌面完整历史和既有重连语义保持不变。
- **测试**: 八屏红绿灯用例让前两个握手故意卡死，验证并发始终不超过两个、其余窗格可见、重试后八个完整终端全部就绪且请求未携带历史截断参数；真实 25 会话采样中全部首屏终端由约 5.85 秒降至约 2.04 秒完成，首次误超时从 6 个降为 0。

### 单屏与多屏切换反复重新加载全部终端

- **现象**: 已经加载完成的六屏、八屏切到单屏后再切回时，每个终端都会重新显示加载状态并重新回放历史，切换耗时随窗格和历史数量增加。
- **根因**: 布局归一化把 `terminalSlots` 截断到当前容量，React 随即卸载被截断窗格；窗格内的 xterm、WebSocket 和 scrollback 都在卸载清理中销毁，扩屏只能重新创建。直接隐藏旧窗格还会让 ResizeObserver 读取到零尺寸并错误 fit；仅添加 HTML `hidden` 属性又会被项目的 `.focus-terminal-pane { display: flex }` 作者样式覆盖，造成单屏仍显示为一列多行。
- **修复**: 自由排列在当前聚焦页内保留已经实际挂载的窗格实例，布局容量只决定可见窗格；缩屏通过专用 `[hidden]` 样式完全移出多余窗格但不卸载旧 xterm，单屏只显示一个终端并占满终端区，扩屏复用原 key、WebSocket 和缓冲区。保留池仍受八窗格上限约束，已删除或被移动的会话会及时清理；零宽或零高期间跳过字体 fit、常规 fit 和 ResizeObserver resize。
- **测试**: 单元红绿灯覆盖保留、当前布局覆盖旧映射、删除会话清理、隐藏样式和零尺寸保护；Playwright 覆盖六屏→单屏→六屏过程中可见窗格数量严格为 `6→1→6`，且终端 WebSocket 创建总数保持六条。

### 聚焦未读完成后返回宫格仍停留在待验收

- **现象**: 打开“待验收”中的完整记录后返回宫格，卡片仍显示在待验收列，直到下一次会话快照到达才移动到可继续。
- **根因**: 聚焦接口异步确认已读，前端没有立即更新当前快照，返回宫格发生在后端确认或 WebSocket 增量之前时会渲染旧状态。
- **修复**: 聚焦入口统一先乐观清除本地 `hasUnreadCompletion`，再异步调用 focus API；后端失败时后续快照仍可恢复真实状态。
- **测试**: `session-snapshot-updates` 覆盖未读状态转为已读，`terminal-preview` 覆盖从待验收聚焦后回到可继续列。

### 窄屏聚焦页返回按钮被右侧会话栏遮挡

- **现象**: 1280px 左右的桌面窗口打开右侧会话栏或 VS Code 后，聚焦页“返回宫格”按钮视觉上仍在标题栏，但实际点击位置被“全部会话”侧栏覆盖。
- **根因**: 标题栏强制单行，固定宽度的布局、输入策略和退出控件在窄主列中溢出到侧栏区域。
- **修复**: 1400px 以下允许聚焦标题栏换行并缩短会话名弹性基准，保证操作按钮留在主列命中区域。
- **测试**: VS Code Web 聚焦抽屉 E2E 覆盖打开、切换侧栏后返回宫格。

### 文件浏览器返回列表后仍残留文本编辑器状态

- **现象**: 文本文件保存后点击“返回文件列表”，再次选中该文件时预览区域仍是编辑器，无法看到最新预览文本。
- **根因**: 返回按钮只收起预览区域，没有清理当前普通文本的 `editorState`；文件复选框已处于选中态时不会再次触发选择事件来覆盖该状态。
- **修复**: 返回文件列表时校验未保存修改并清理普通文本编辑状态，再收起预览区域；保存后的再次选择恢复最新文件预览。
- **测试**: `file-browser` 本地保存后返回列表并重新预览，远端编辑流程统一显式点击“编辑”。

### Markdown 全屏预览无法使用鼠标滚轮

- **现象**: 桌面文件系统进入 Markdown 全屏渲染预览后，鼠标滚轮不再滚动文档；切到源码编辑界面时滚动仍可能正常。
- **根因**: 全屏预览通过 Portal 挂载到 `document.body`，终端的 document-level 滚轮兜底只排除了旧文件弹窗，没有识别新的 `.file-browser-fullscreen-preview`。当全屏层覆盖在终端坐标上时，后台 xterm 在捕获阶段接管并取消了滚轮默认行为。
- **修复**: 将全屏文件预览加入终端滚轮阻断目标；预览层内部继续由 Markdown 渲染区、目录或编辑器各自处理滚动，事件不再穿透到后台终端。
- **测试**: `terminal-wheel.test.ts` 红绿灯覆盖全屏预览目标隔离；`file-browser.spec.ts` 覆盖长 Markdown 进入全屏后真实鼠标滚轮令渲染区 `scrollTop` 增长。

### VS Code iframe 点击后焦点被终端抢回

- **现象**: 桌面聚焦页打开内嵌 VS Code 后，鼠标点击编辑器短暂可用，但稍后焦点会回到 xterm，编辑器无法稳定输入或选择内容。
- **根因**: 终端组件本身已保护 iframe，聚焦视图在会话/窗格状态更新后的兜底聚焦 effect 却只排除了表单和弹窗，没有排除 iframe；它直接调用全局终端聚焦函数，绕过了终端内部保护。
- **修复**: 聚焦视图把 iframe 纳入外部文本输入面保护；只有当前焦点不在 iframe、表单、可编辑区或弹窗时，布局变化才会兜底恢复终端焦点。用户重新点击终端时仍可正常取得输入权。
- **测试**: `AgentFocusView.test.ts` 锁定 iframe 保护选择器，`vscode-web.spec.ts` 在点击编辑器后等待超过被动焦点修复周期，再验证 iframe 持续持有焦点并可输入。

### 版本更新提示长期占据大块界面

- **现象**: 本地源码或远程分支出现新版本时，固定在顶部的大横幅持续遮挡标题栏和工作区；用户希望不更新时只保留一个很小的提醒。
- **修复**: 可用更新、远程更新、冲突和检查失败统一收敛为左上角 `24px` 状态灯；不点击时保持小尺寸，点击后直接执行更新、拉取或重试。警告状态用橙色区分，版本与失败原因通过 `title` 和 `aria-label` 保留。
- **测试**: `AppUpdateBanner.test.ts` 覆盖本地、远程和冲突三种紧凑状态及单击动作入口；热更新 Playwright 用例同步验证状态灯、常驻语义和恢复流程。

### Markdown 预览文字选区很快消失

- **现象**: 桌面文件系统的 Markdown 渲染预览可以短暂选中文字，但选区很快被清除，难以复制或对照文档内容。
- **根因**: 不是 Markdown 内容重新请求或重新解析，而是聚焦视图在会话/布局更新时调用终端兜底聚焦；该入口会立即、动画帧和零延时连续三次聚焦 xterm 隐藏 textarea，浏览器因此折叠已有文档选区。
- **修复**: 终端兜底聚焦的三次尝试都先检查当前 `Selection`；存在非折叠文字选区时不转移焦点。用户点击终端后选区自然折叠，后续回调仍能正常恢复终端输入。
- **测试**: `terminal-focus.test.ts` 红绿灯复现三次 focus 抢占并验证选区保护；`file-browser.spec.ts` 在 Markdown 预览建立文字选区、等待超过焦点修复周期后确认选区仍保留。

### 图片发送窗口难以取消且 Codex 始终提示不支持附件

- **现象**: 电脑端选中或粘贴图片后，确认窗口没有直观的“取消”操作；点击发送则返回 Codex 不支持图片附件，无法把截图交给当前对话。
- **根因**: 首版只提供关闭图标、遮罩和 Escape，缺少显式取消按钮；当前 Codex CLI 的 `queue --help` 虽声明 `-i`，实际执行 `codex queue ... -i` 却返回不支持图片附件。
...
- **现象**: 电脑端从终端卡片打开“完整记录”会出现覆盖页面的大弹窗，阅读记录时无法同时观察和操作终端，也没有复用已经成熟的文件系统分栏布局。
- **根因**: 完整记录的打开状态和 Portal 弹窗都由 `AgentFocusView` 局部持有，桌面与手机只能共用同一种模态呈现，无法参与 App 层全局侧栏工具的互斥、跟随和折叠逻辑。
- **修复**: 将完整记录加入 App 层 `SidePanelTool`，电脑端复用文件系统的可调宽、可折叠左侧工作区，右侧终端保持可见；共享记录组件增加面板呈现并保留 Markdown、分页和刷新能力，移动端继续使用原 Portal 弹窗。面板建立键盘隔离边界，点击记录或按钮不会把按键误送入终端；多屏切换当前输入会话时面板随之更新数据源。关闭记录或再次点击已激活入口时，不关闭整个侧栏，而是回退到当前会话的文件系统。
- **测试**: `DesktopTranscriptPanel.test.ts` 锁定桌面侧栏集成，`AgentTranscriptDialog.test.ts` 覆盖非模态语义与键盘隔离，`AgentFocusView.test.ts` 覆盖活动按钮和终端按键转发保护。

### 桌面完整记录无法拖选对话文字

- **现象**: 完整记录迁入桌面左侧工作区后，鼠标拖选历史对话时选区无法形成或立即消失，不能使用浏览器原生复制。
- **根因**: 记录面板虽然已经隔离键盘转发，但 `TerminalView` 自己的 `focusout` 和每 500ms 被动焦点修复只保护输入框、iframe 与模态框，不认识新的非模态阅读面；xterm 隐藏 textarea 因此重新取得焦点并折叠浏览器选区。
- **修复**: 指针进入记录正文时由可聚焦阅读区接管非输入焦点，`TerminalView` 将统一的键盘隔离边界识别为受保护外部焦点；所有被动终端聚焦在检测到非折叠文档选区时直接停止。记录正文显式启用原生文字选择，并提供与界面配色一致的选区样式；用户重新点击终端仍会立即恢复输入。
- **测试**: `AgentTranscriptDialog.test.ts` 红绿灯锁定阅读区焦点接管、终端保护选择器和 `user-select: text`；`terminal-focus.test.ts` 覆盖非折叠选区识别及既有延迟聚焦保护。

### 飞书富文本回复没有继续对应 Codex

- **现象**: 用户明确回复了已绑定的任务完成卡片，但带格式或多段内容被飞书标记为 `post` 后没有进入 Codex 队列。
- **根因**: `lark-cli` 已将 `post` 内容解码为可读文本，回复服务仍只允许消息类型 `text`，因此在绑定检查前将事件归类为不可信。
- **修复**: 仅对可信私聊且带有效回复关系的 `text`/`post` 放行，二者继续共用长度、控制字符、会话可用性和去重校验；图片等非文本类型保持拒绝。
- **测试**: `feishu-reply-command-service.test.ts` 红绿灯覆盖富文本完整投递，以及含危险控制字符的富文本拒绝。

### 回复上一条飞书用户消息没有继续对应 Codex

- **现象**: 首次回复任务完成卡片可以进入 Codex，但随后在飞书中回复这条已经成功发送的用户消息时，看板没有收到内容。
- **根因**: 回复服务只为机器人通知卡片保存会话绑定；第二层消息的 `reply_to` 指向第一层用户回复，因此无法直接解析到原卡片绑定并被归类为 `ignored_unbound`。
...
- **现象**: 已不存在的 tmux 会话在恢复失败后仍显示“服务已更新，等待恢复 tmux 会话”，容易误以为恢复仍在进行。
- **根因**: 恢复器只返回失败结果给恢复提示，没有把失败原因回写到对应会话的终端预览。
- **修复**: 恢复失败时回写明确的“恢复失败”状态和原因；仍存在的 tmux 会话继续按原流程重连，成功后由在线输出覆盖提示。
- **测试**: `managed-session-restorer.test.ts` 覆盖找不到 tmux 与重连异常都会触发失败回写回调。

### 飞书普通新消息可能被陈旧父消息字段误认为回复

- **现象**: 用户在机器人私聊中直接发送普通新消息时，担心它被路由到最近选择的 Codex 对话并中断当前任务。
- **根因**: 回复入口过去强制校验 `reply_to`，但没有同时要求飞书回复链的 `root_id`；若事件适配层异常携带一个仍有本地绑定的旧父消息 ID，单凭父绑定不足以证明用户执行了“回复”。
- **修复**: 每条可执行消息现在必须由合法且已绑定的 `root_id` 锚定原通知；父绑定存在时必须与根绑定指向相同 session/thread。没有回复根的普通新消息会在解析会话和调用 `codex queue` 前拒绝。
- **测试**: `feishu-reply-command-service.test.ts` 覆盖“普通新消息误带陈旧已绑定 `reply_to`、但没有 `root_id`”时不得解析或写入 Codex，并保留一层/多层回复和升级前根回退测试。
...
- 2026-09-23：远程点击「快捷回复」后无响应。飞书先因独立 `select_static` 使用 `required` 拒绝创建面板；面板创建成功后，真实下拉回调又省略 `action_name`，被后端误拒绝。移除两个独立下拉框的 `required`，通过动作值与卡片绑定识别选项，容忍缺失的 `action_name`；补充结构与真实回调形状测试，并用飞书私聊诊断发送确认卡片可创建。
- 2026-09-23：完成通知卡片把「快捷回复」「查看完整记录」及引用文件操作作为独立顶层按钮，客户端顺序换行，操作区显得分散。现在统一放进横向 `column_set`，窄屏和文件较多时自动换行；回调值与单个按钮尺寸保持不变。
- 2026-09-24：完成通知的所有操作都在同一个 `flow` 分栏里，文件按钮可能把「快捷回复」「查看完整记录」挤到不同视觉行，不能保证卡片最后一行是这两个入口。现在每个带文件名的查看按钮各占一行，最后一组使用不换行的两列布局固定放置快捷回复和完整记录；无文件时只保留最后一组，回调值不变。脚本红绿灯测试覆盖多文件行和最终顺序。
...
- 2026-09-24：聚焦视图的显示页面虽分别保存了窗格会话，切回已有页面时却把上一页的聚焦会话当成必选项，覆盖当前页原有终端并自动写回；分组排列的当前输入终端也只放在组件共享状态。现仅为空白页优先填入聚焦会话，已有页恢复自身窗格；分组当前输入终端按页持久化，页面恢复期间不让未变化的外层聚焦状态再次覆盖。红绿灯测试覆盖跨页终端恢复、分组输入恢复及侧栏联动。
- 2026-09-24：本机 `tmp` tmux 卡片登记为 `shell` 且没有 pane ID，实际唯一 pane 运行 `claude`；原同步器只识别已绑定 pane，使飞书把终端静默视为 Shell 完成，并把 Claude 告警或 tmux 状态栏当“完整输出”。现只在卡片和 pane 均唯一、pane ID 合法时补齐绑定并同步命令；未绑定的本机 tmux 卡片禁止终端预览兜底，已绑定普通 Shell 行为不变。运行态确认 `tmp` 已识别为 `claude` 并绑定 pane；该 Claude 当前提示 transcript 写入失败、JSONL 缺失，因此真实完成仍需等待其记录恢复。
- 2026-09-24：远端 `24_hermes` 的 tmux 3.0a 仍有输出，但看板 SSH PTY 于 20:27 退出并显示 `Process exited with code 1`；附着命令先执行新版本的 `terminal-features` 选项，旧版返回 `invalid option` 后使整条 tmux 命令中止。现从附着命令中移除该可选设置，只执行各版本支持的鼠标、历史及附着命令；红绿灯测试以不支持该选项的假 tmux 验证仍能附着。
