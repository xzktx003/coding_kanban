...
当后端已从结构化记录确认精确 Codex thread 时，每张通知分片底部都会附带一个 `tiny` 尺寸的「查看完整记录」按钮。按钮回调只包含固定动作名，不包含 session ID、thread ID、工作目录或主机；后端根据被点击卡片的 `message_id` 读取本地短期绑定，校验配置用户和私聊，再确认该看板会话仍处于同一 Codex thread。校验通过后直接发送会话工作区的记录卡片，可查看最近公开对话、继续翻页或导出完整 Markdown。会话已经切换、离线、绑定缺失，或“飞书回复继续执行”关闭时拒绝打开；无法确认精确 thread 的非 Codex 降级通知不显示该按钮。该入口复用现有 `card.action.trigger` 长连接，不需要新增飞书权限。
