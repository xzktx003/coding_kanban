...
完成通知自动识别 `$...$`、`$$...$$`、`\(...\)`、`\[...\]`，在本机用仓库已有的 KaTeX 与 Playwright Chromium 排版、截图，再由同一个 `lark-cli` 机器人上传并嵌入 Markdown。公式图片出现时卡片使用常规宽度，普通正文仍可选择和复制。行内公式也使用图片，飞书可能将图片独立成行，不保证与网页一样的行内基线；图片本身不能像文字一样复制公式。

在发送通知的 Kanban 主机安装 Chromium 及系统依赖：`pnpm exec playwright install --with-deps chromium`，并为机器人开通图片上传权限（`im:resource:upload` 或接口允许的 `im:resource`）。这是可选增强，缺少浏览器、权限不足、公式语法错误、超限或上传失败时保留对应的公式源码，继续发送通知。无需修改 Codex/Hermes，也无需新建个人配置。

识别发生在隐私裁剪之后、长正文分片之前；代码围栏、行内代码、缩进代码、转义美元符和常见金额不转换。相同公式在一次通知中只上传一次，每次最多 20 个不同公式，每个最多 4000 字符，额外公式保留源码。公式整批处理复用 `FEISHU_NOTIFY_TIMEOUT_MS` 作为总时间预算（正式通知默认 10 秒），超时保留剩余公式源码；后续消息发送仍有自己的重试预算。图片限制为 1500×3000 像素，极端尺寸不生成难以阅读的缩略图。图片引用跨卡片边界时保持完整。使用 `trust: false`，浏览器禁止联网，字体来自本地，临时图片发送处理后清理，不调用外部公式服务。

此增强只改变卡片正文，不改变任务完成判定、通知开关、消息幂等键或私聊回复绑定。历史卡片不会自动重绘。
