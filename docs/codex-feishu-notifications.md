...
完成通知卡片使用 Card 2.0 `fill` 宽度，随飞书当前会话的可用内容区域自适应。桌面端放大会话窗口时卡片可以同步变宽，移动端则填充设备可用宽度；卡片中的 `tiny` 操作按钮仍保持小尺寸。历史卡片保留发送时的宽度配置，不会自动切换。
...
当后端已从结构化记录确认精确 Codex thread 时，每张通知分片底部都会附带一个 `tiny` 尺寸的「查看完整记录」按钮。按钮回调只包含固定动作名，不包含 session ID、thread ID、工作目录或主机；后端根据被点击卡片的 `message_id` 读取本地短期绑定，校验配置用户和私聊，再确认该 thread 仍存在于对应的看板/tmux 会话。校验通过后直接发送原始分屏的会话工作区记录卡片，可查看最近公开对话、继续翻页或导出完整 Markdown；切换活动 pane 不会改变目标。该入口是只读的：飞书通知已配置并开启即可使用，不要求打开“飞书回复继续执行”。会话已经切换、离线、绑定缺失或原分屏已关闭时拒绝打开并发送说明；无法确认精确 thread 的非 Codex 降级通知不显示该按钮。回调同时接受 `action_value` 为 JSON 字符串或对象。该入口复用现有 `card.action.trigger` 长连接，不需要新增飞书权限。
