...
完成通知卡片使用 Card 2.0 `fill` 宽度，随飞书当前会话的可用内容区域自适应。桌面端放大会话窗口时卡片可以同步变宽，移动端则填充设备可用宽度；卡片中的 `tiny` 操作按钮仍保持小尺寸。「快捷回复」「查看完整记录」和引用文件按钮位于同一个横向操作组，窄屏或文件较多时按飞书布局自动换行。历史卡片保留发送时的宽度和按钮布局，不会自动切换。
