...
本机 Codex 完成回复中的明确 Markdown 文件或图片链接还会经过安全解析：仅当目标是当前项目目录内真实存在的普通文件时，才把原本无法在飞书打开的本地 URL 改成代码样式相对路径，并在首张结果卡片追加最多 5 个 `tiny`「查看 文件名:行号」按钮。HTTP(S) 链接、代码围栏内示例、项目外路径、目录、符号链接、隐藏文件、凭证类文件和 SSH 会话均不建立入口。私聊中的 PNG、JPEG、WebP、GIF 引用会在发送前再次校验项目边界、非符号链接、扩展名、真实文件签名和 10 MB 上限；验证通过后用机器人身份上传为飞书消息图片，并在 Card 2.0 `img` 组件中直接展示、支持点按放大。上传失败时只省略内嵌图片，原通知及文件查看按钮继续发送。按钮回调只携带引用序号，真实相对路径随通知绑定保存在本机；点击后再次校验配置用户、私聊、消息绑定、精确 thread、工作目录和文件边界，再通过会话工作区发送文本预览，无法预览的文件提供受限下载。该能力不创建公网或内网 HTTP 文件 URL，手机无需直连 Kanban。
