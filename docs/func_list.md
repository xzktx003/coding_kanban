...
- 聚焦视图侧栏单击会立即把会话切换到当前主输入窗格；双击不会重复切换。单屏模式保留当前会话及最近两个 xterm 实例，来回切换时复用现有终端、WebSocket 和 scrollback；聚焦确认使用有序的轻量 `204` 请求，最终状态通过现有会话增量通道同步，不再用完整 HTTP 快照触发全部卡片重渲染。
