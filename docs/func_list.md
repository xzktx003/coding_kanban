...
- 聚焦视图顶栏新增“变更”面板，明确分为“本次任务”和“当前工作区”两个独立 Diff 语义，并默认打开信息更完整的当前工作区。当前工作区基于可信会话工作目录读取 Git 已跟踪文件的修改、暂存、新增、删除、重命名和冲突，也逐文件展示未被 `.gitignore` 排除的 `??` 新文件及其 `/dev/null → 文件` Diff，支持文件筛选、增删统计、复制路径和引用到输入框。每个 `@@` 改动块在鼠标悬停或键盘聚焦时显示“还原此改动”，经不可撤销确认后只反向应用该块并保留同文件其他修改；后端同步处理该块在暂存区和工作区中的内容，新增或未跟踪文件的唯一改动块还原后可能删除文件。本次任务首版仅支持本机 Codex，基于最新用户任务之后的 `apply_patch` 记录生成，并显示中可信提示；没有可靠记录时明确提示不可归因，不用当前 Git Diff 冒充 Codex 改动。
