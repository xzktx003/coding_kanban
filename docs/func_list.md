...
- 每个监控窗格标题栏的会话切换器按看板当前分组顺序展示可选会话，并复用稳定分类色、组名和数量；当前会话显示“当前”，已在其他窗格展示的会话显示对应窗格编号且不可重复选择，未分组会话保持独立分区。分组内容超出弹层高度时由外层列表统一滚动，不裁切组内选项；当前分组标题到达顶部后保持吸顶，直到下一分组标题向上滚动并替代它。
...
- 聚焦视图顶栏新增“变更”面板，明确分为“本次任务”和“当前工作区”两个独立 Diff 语义，并默认打开信息更完整的当前工作区。当前工作区基于可信会话工作目录读取 Git 已跟踪文件的修改、暂存、新增、删除、重命名和冲突，也逐文件展示未被 `.gitignore` 排除的 `??` 新文件及其 `/dev/null → 文件` Diff，支持文件筛选、增删统计、复制路径和引用到输入框。每个 `@@` 改动块在鼠标悬停或键盘聚焦时显示“还原此改动”，经不可撤销确认后只反向应用该块并保留同文件其他修改；后端同步处理该块在暂存区和工作区中的内容，新增或未跟踪文件的唯一改动块还原后可能删除文件。本次任务支持本机 Codex 及运行 Codex 的本地 tmux，会先解析卡片对应的精确 Codex session，再按最新 `turn_id` 直接映射原生 `FileChange`（新增、删除、修改和重命名），旧版记录继续以最新用户任务之后的 `apply_patch` 作为兼容兜底；没有可靠记录时明确提示不可归因，不用当前 Git Diff 冒充 Codex 改动。
- 手机终端页增加全屏“变更”视图，支持本次任务/当前工作区切换、下拉选择变更文件、简化统一 Diff、复制路径、引用到输入框和当前工作区逐改动块还原；触屏设备不依赖悬停，直接显示改动块还原按钮；不提供 stage、commit、merge 等仓库级写操作。
- 手机端默认入口为手机 Agent 工作区首页，按“需响应 / 待验收 / 执行中 / 可继续”的注意力顺序展示独立会话轻量摘要；点击会话后才进入当前会话终端控制页，避免列表为每个会话创建真实终端或 WebSocket。
...
- 提供手机端终端控制页，默认入口为 `/?view=mobile`，兼容 `/mobile`、`/m` 和 `#/mobile`；复用现有会话和终端通道，以单会话全屏终端、底部快捷键条和多行输入框为主，解决手机缺少 `Ctrl+C`、`Esc`、方向键、稳定软键盘输入和终端历史滑动的问题。手机终端在挂载时完成一次不启用 stdin 的光标初始化，并使用高对比度下划线光标；输入框或快捷键持有页面焦点时仍显示 TUI 当前编辑位置，初始化后会立即归还此前的页面焦点；快捷键条提供”说明”按钮，可弹出各快捷键用途说明；说明弹窗支持 `aria-modal`、`aria-labelledby`、Tab 聚焦陷阱和 Escape 关闭。
...
- 手机端快捷键条发送通用终端控制字符，不提供 tmux 专用快捷键专区；`Ctrl+C/D`、`Esc`、`Tab`、`Shift+Tab`、方向键、`Ctrl+L`、`Ctrl+U/W/K/Y/A/O/E` 等控制键按原始 stdin 转发，不会被额外追加 Enter，并覆盖 Claude / Copilot CLI 常用的焦点切换、面板打开和行编辑快捷键。工具栏提供一次性 `Shift` 修饰键，激活后可与下一次 Tab、Enter 或方向键组合，发送后自动复位。
...
- 多终端窗格标题栏的会话切换器复用同一分组顺序、名称和稳定分类色；侧栏分组即使处于折叠状态，切换器仍展示该组全部可选会话，避免折叠视图阻断窗格切换。
- 多终端窗格的会话切换器打开后支持搜索会话名和分组名；命中分组名时保留该组全部会话，命中会话名时只显示对应会话，并继续保留分组层级、占用窗格标记和当前输入状态。
