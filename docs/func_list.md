...
- 手机端默认直接进入“当前会话”终端控制页，立即展示已选会话；当前会话选择条使用醒目的聚焦终端样式并同时显示状态。底部“看板”仍按“需响应 / 待验收 / 执行中 / 可继续”的注意力顺序展示全部会话：分组标题和卡片复用桌面端的稳定状态色、强调条及数量徽标，卡片在手机宽度内紧凑显示任务/回复、Git 分支与增删统计、末尾三行轻量终端内容。卡片只消费已有会话快照，不为每个会话创建真实终端、WebSocket 或额外摘要请求。
