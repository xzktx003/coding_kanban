...
- 支持文本预览和编辑保存。
- `.md` / `.markdown` 文件单击或双击后默认进入编辑模式，预览和分屏必须由用户手动切换；预览支持 GitHub Flavored Markdown、标题、列表、任务列表、表格、代码块、安全外链，以及 `$...$` / `\(...\)` 行内公式和 `$$...$$` / `\[...\]` 块级 LaTeX 公式，内嵌 HTML 不执行。Markdown/KaTeX 渲染引擎按需加载，预览正文记忆化，分屏编辑使用延迟内容刷新；独立弹窗打开时不再保留后方重复预览实例，仅在显式保存时写入文件。
