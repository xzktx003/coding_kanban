...
- 手机端会话标题区使用单实例页面内列表切换终端，列表支持点外部或 Escape 关闭，不依赖会被实时快照反复触发的浏览器原生选择层；可选会话固定按“可继续 → 执行中 → 其它状态”分组，每组按名称的中文拼音或字母顺序稳定排列，不跟随活动时间跳动。“完整记录”“变更”和“文件”保持在同一行操作组，其中完整记录与桌面聚焦视图复用同一记录弹窗，“文件”直接打开当前会话的只读文件系统并可一键返回终端。
