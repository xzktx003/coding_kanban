...
- 聚焦页标题栏提供“完整记录”入口：本机会话可读取 `~/.codex/sessions` 中的 Codex JSONL，按最新记录在前的倒序展示用户消息、助手回答及非 `exec` 工具记录，不受 TUI 光标移动、擦除行或终端重绘影响；`exec` 的调用与输出均不展示。优先按 Codex session ID 精确匹配，缺少 ID 时按工作目录选择最近记录并明确标注推断来源。工具输出默认折叠，可按需展开和手动刷新；首版不读取远端主机文件。
