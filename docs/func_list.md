...
- 手机端提供固定主导航：看板、活动、当前会话、项目/文件；活动按最近更新时间展示摘要，项目/文件按项目或工作目录聚合会话入口。
...
- 手机端会话标题区使用单实例页面内列表切换终端，列表支持点外部或 Escape 关闭，不依赖会被实时快照反复触发的浏览器原生选择层；“完整记录”和“变更”保持在同一行操作组，其中完整记录与桌面聚焦视图复用同一记录弹窗。
