...
- 聚焦视图顶栏新增“变更”面板，明确分为“本次任务”和“当前工作区”两个独立 Diff 语义，并默认打开信息更完整的当前工作区。当前工作区基于可信会话工作目录读取 Git 已跟踪文件的修改、暂存、新增、删除、重命名和冲突，也逐文件展示未被 `.gitignore` 排除的 `??` 新文件及其 `/dev/null → 文件` Diff，支持文件筛选、增删统计、复制路径和引用到输入框。本次任务首版仅支持本机 Codex，基于最新用户任务之后的 `apply_patch` 记录生成，并显示中可信提示；没有可靠记录时明确提示不可归因，不用当前 Git Diff 冒充 Codex 改动。
- 手机终端页增加全屏“变更”视图，支持本次任务/当前工作区切换、下拉选择变更文件、简化统一 Diff、复制路径和引用到输入框；不提供 stage、discard、commit、merge 等修改 Git 状态的操作。
- 手机端默认入口为手机 Agent 工作区首页，按“需响应 / 待验收 / 执行中 / 可继续”的注意力顺序展示独立会话轻量摘要；点击会话后才进入当前会话终端控制页，避免列表为每个会话创建真实终端或 WebSocket。
- 手机端提供固定主导航：看板、活动、当前会话、项目/文件；活动按最近更新时间展示摘要，项目/文件按主机和项目目录聚合会话入口，并可直接进入只读文件系统。文件系统复用本地/SSH 文件接口，支持目录导航、当前目录搜索、隐藏文件开关、可纵向触摸滚动的文本与图片预览、复制路径；Markdown 与电脑端复用安全的 GFM/KaTeX 渲染链并适配窄屏表格、代码和图片，同时提供“渲染 / 源码”查看方式切换。大型 UTF-8 文件按 64 KiB 窗口流式读取，可用“上一段 / 下一段”继续阅读，切换查看方式或翻段时页面始终只保留当前窗口。该视图不挂载终端，也不提供修改或删除操作。

- 聚焦主终端使用 xterm.js 渲染，后端通过 WebSocket 发送 scrollback replay 与实时输出。
- 聚焦页标题栏提供“完整记录”入口：本机会话可读取 `~/.codex/sessions` 中的 Codex JSONL，按最新记录在前的倒序展示用户消息、助手回答及非 `exec` 工具记录，不受 TUI 光标移动、擦除行或终端重绘影响；`exec` 的调用与输出均不展示。用户与 Codex 发言统一使用安全 Markdown/GFM/KaTeX 渲染，并在接近可视区域时才懒加载；工具输出保持折叠的等宽原文，可按需展开和手动刷新；两类正文均跟随全局终端字号。优先按 Codex session ID 精确匹配，缺少 ID 时按工作目录选择最近记录并明确标注推断来源；首版不读取远端主机文件。
...
- 手机端会话标题区使用单实例页面内列表切换终端，列表支持点外部或 Escape 关闭，不依赖会被实时快照反复触发的浏览器原生选择层；“完整记录”“变更”和“文件”保持在同一行操作组，其中完整记录与桌面聚焦视图复用同一记录弹窗，“文件”直接打开当前会话的只读文件系统并可一键返回终端。
