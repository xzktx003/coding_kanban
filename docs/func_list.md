...
- 支持文本预览和编辑保存。
- `.md` / `.markdown` 文件默认渲染 GitHub Flavored Markdown，支持标题、列表、任务列表、表格、代码块、安全外链，以及 `$...$` / `\(...\)` 行内公式和 `$$...$$` / `\[...\]` 块级 LaTeX 公式；内嵌 HTML 不执行。单击文件在预览区查看，双击打开可拖动调整宽高的大窗口，并可在“预览 / 编辑 / 分屏”间切换，编辑内容和公式实时渲染但仅在显式保存时写入文件。
