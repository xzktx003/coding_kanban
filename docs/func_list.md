...
- 手机端快捷键条发送通用终端控制字符，不提供 tmux 专用快捷键专区，也不把低频按键收进“更多”；所有按键保持在同一横向常驻行，按 `Shift / Esc / Ctrl+C / 方向键 / 退格 / Tab / Enter / 组合键 / 说明` 排列。工具栏提供一次性 `Shift` 修饰键；方向键和退格支持长按连续发送，每次等待上一笔 stdin 请求完成后再继续，松手后不会残留输入队列。
...
- 手机端快捷键工具栏保持单行横向选择器布局，用户可左右滑动选择全部常驻快捷键；按键触摸高度至少 44px，重复键长按时阻止系统文本选择和触摸呼出菜单。字号持久化到 localStorage，reload 后保持上次缩放级别。
