...
- 聚焦视图顶栏新增“变更”面板，明确分为“本次任务”和“当前工作区”两个独立 Diff 语义；当前工作区基于可信会话工作目录读取 Git 已跟踪文件的修改、暂存、删除、重命名和冲突，不展示 `??` 未跟踪生成产物，支持文件筛选、增删统计、复制路径和引用到输入框。本次任务首版仅支持本机 Codex，基于最新用户任务之后的 `apply_patch` 记录生成，并显示中可信提示；没有可靠记录时明确提示不可归因，不用当前 Git Diff 冒充 Codex 改动。
