...
- 聚焦视图支持单屏、左右双屏、上下双屏、左中右三屏、四屏、六屏、八屏终端监控模式；多窗格只用于同时观察，键盘输入只发送到带高亮边框和“当前输入”醒目角标的一个窗格，不支持广播输入。点击窗格、选择不同会话或点击侧栏会话后，当前输入窗格、顶部标题以及文件/VS Code 工具上下文保持同步；在非输入窗格重复选择其当前会话只关闭菜单，不抢占输入。
