...
- 快捷键条是单行横向滑动选择器，所有快捷键常驻而不增加二级“更多”菜单；高频的 `Esc`、`Ctrl+C`、方向键和退格前置，一次性 `Shift` 与其余组合键继续保留。方向键和退格支持短按一次、长按串行重复，上一笔 stdin 请求完成后才安排下一笔，避免慢网络产生松手后继续执行的积压；用户可点击末尾“说明”查看每个快捷键的作用。
