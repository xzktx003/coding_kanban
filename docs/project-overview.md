...
变更审查将“Codex 本次任务记录的文件操作”和“当前 checkout 的 Git 工作区状态”建模为两种独立数据产品。两者只共享 Diff 文件/行的视觉渲染，不共享归因、统计和缓存语义；任务记录无法可靠归因时显示不可用，禁止把当前 `git diff` 冒充 Codex 改动。手机端使用紧凑下拉框选择变更文件，避免完整文件列表挤占 Diff 空间；桌面和手机的文件内容均可进入独立全屏 Diff，并支持退出、复制路径和引用文件。
...
- 当前会话页面采用单会话全屏终端：顶部显示桌面入口、当前会话状态和单实例页面内会话选择器，“完整记录 / 变更”在同一行操作组；中间为 xterm，底部为快捷键条和多行输入框。会话列表不使用浏览器原生选择层，实时快照更新时不会重复堆叠，支持点外部或 Escape 关闭。
