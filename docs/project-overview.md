...
「任务总览」菜单（`kanban_task_overview`）复用同一服务和事件订阅，以面板模式区分总览与原对话入口；从注册表读取未隐藏会话的状态统计和有界最近输出摘要，每页 10 个，只有当前页在线可控且身份明确的 Codex 可提交指令。翻页与刷新保留服务端模式及来源校验，不增加状态轮询或完成判定逻辑，也不修改 Codex/Hermes。
