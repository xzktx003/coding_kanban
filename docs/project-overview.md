...
飞书个人快捷回复在现有控制面板中增加 `quick_replies` 模式：固定菜单先选 Codex 会话，再选模板；下拉事件使用卡片延迟更新 token 在原消息内切换到预览编辑，选择本身不投递。完成通知按钮通过持久消息绑定锁定原 thread。模板由 `FeishuQuickReplyStore` 从源码目录下 Git 忽略的本地 JSON 或 `.env` 指定路径读取，不将个人正文写入仓库或回调；版本摘要阻止旧面板发送已变化的模板。预览编辑保留目标并只改变本次正文，明确提交后复用 Codex 原生文字队列。详见 [快捷回复配置与架构](feishu-quick-replies.md)。

Codex 完成通知沿结构化 `task_complete` 关联本轮真实用户问题，通过可选 `userQuestion` 字段传入发送脚本；不从会话摘要或任意历史输入猜测。Goal 自动续轮跳过内部提示；记录扫描窗口内来源不明时仅发送结果。问题与回复分区展示，长问题另用折叠卡片发送，仍共享原会话回复绑定。完成卡片使用 Card 2.0 `fill` 宽度随飞书会话可用区域自适应；每张能精确定位 Codex thread 的通知卡片带一个 `tiny`「查看完整记录」按钮，复用现有 `card.action.trigger` 与会话工作区展示公开记录、分页和完整导出。该按钮是只读入口：飞书通知开启即可使用，不依赖“飞书回复继续执行”；文件工作区和向 Codex 发送指令仍受回复控制保护。不新增事件订阅或完成判定状态。本机会话回复中的明确 Markdown 文件/图片链接由后端限制到当前项目真实普通文件，改为非 URL 的相对路径，并在首张私聊结果卡片提供最多 5 个受消息绑定保护的直接查看按钮；安全图片由发送脚本再次校验后上传为飞书资源，并通过 Card 2.0 `img` 原生展示，上传失败只取消内嵌图而不影响完成通知。

Claude 完成通知独立读取 Claude JSONL，只在有文本的 assistant 最终记录明确结束且没有待执行工具调用时触发；初始记录只作基线，避免补发旧任务。卡片绑定独立 Claude transcript UUID，并提供相同的只读完整记录分页/导出入口；该身份不写入 Codex thread 字段，也不启用 Codex 回复或本地文件快捷入口。本机及 SSH 记录均通过对应 Claude JSONL reader 获取。已登记的本机 tmux 会话在服务就绪时及每 5 秒按 pane ID 与 tmux session 名核对当前命令，仅在身份匹配且命令变化时更新注册表的 `agentKind`，使启动后切入 Claude 的会话进入正确的读取与通知路径；远端会话保持独立。完成探测、内容解析和卡片 Agent 类型共用 Claude 可执行命令识别函数，涵盖 `claude.exe` 等实际命令名。
...
「任务总览」菜单（`kanban_task_overview`）复用同一服务和事件订阅，以面板模式区分总览与原对话入口；在飞书能力已配置并开启时，即使“飞书回复控制”关闭，也从注册表读取未隐藏会话的状态统计和有界最近输出摘要，每页 10 个并支持翻页/刷新。回复控制开启后，当前页在线可控且身份明确的 Codex 才会显示指令和工作区入口。翻页与刷新保留服务端模式及来源校验，不增加状态轮询或完成判定逻辑，也不修改 Codex/Hermes。
...
飞书完成卡片直接保留 Markdown/LaTeX 公式源码，不再启动 KaTeX/Chromium 或上传公式图片。发送链路只负责正文清理、分片和 Card 2.0 投递，不改变任务完成判定与回复绑定。详见 [公式源码](codex-feishu-notifications.md#公式源码)。
...
- 顶栏分组展示：左侧是“电脑端 Coding Kanban”、会话数量徽标和“手机端 Coding Kanban”切换入口，中间保留新建会话、扫描、文件、VS Code 等高频入口，右侧提供统一“设置”、全屏和折叠入口。电脑端顶栏、主内容外框和聚焦分屏使用更紧的边距，把面积留给多个 session 终端；当前输入窗格用小号徽章标识，其它窗格保留「设为输入」。
...
- Agent 类型：`shell`、`codex`、`claude`、`copilot`。新建会话默认选中 `shell`。
- 启动方式：
  - direct：直接启动命令。
...
聚焦主终端由 xterm.js 渲染，后端通过 `node-pty` 和 WebSocket 驱动。终端预览模式默认使用轻量文本预览，宫格卡片和聚焦右侧栏不创建真实 xterm 实例，也不打开 `/terminal` WebSocket；用户可从顶栏切换到完整预览模式，恢复旧版小终端行为。聚焦视图通过一个 `屏幕布局` 菜单提供单屏、左右双屏、上下双屏、左中右三屏、上中下三屏、四屏、六屏和八屏监控布局，用于显式打开最多 8 个实时终端窗格。响应式重排以主终端容器而非浏览器视口为基准，多列结构保持到容器小于 `720px` 才切换单列，避免六屏在仍可用的桌面宽度下过早变成六行。单列模式下，左右双屏和左中右三屏的每个窗格保持容器全高，上下双屏和上中下三屏分别按两行、三行均分，四/六/八屏窗格保持容器半高，超出部分由终端布局自己的纵向滚动条浏览。
...
`activeSlotId` 与 App 级 `focusedId` 保持有条件分离：没有文件、VS Code 或变更侧栏时，切换监控窗格只更新本地输入槽位和活动标题，不触发 `/focus` 或全局会话快照；侧栏工具已打开时才同步 focused session，以保持文件/编辑器上下文跟随活动窗格。该解耦只适用于 focused session ID 未变化的多屏内切换；看板进入、侧栏选择或其他外部操作改变 focused session 时，聚焦视图必须激活其可见窗格并转移唯一输入权。

窗格标题栏的 `TerminalSessionSwitcher` 从同一 `SessionGroupState` 派生分组列表，并复用既有折叠状态机制，但使用独立的 `terminal-switcher` 作用域，因此主页看板、聚焦侧栏与切换弹层互不联动。分组标题是可键盘操作的展开/折叠按钮，收起后仍显示组名和数量；通过 portal 挂载到页面根层，避免被终端窗格的裁切边界截断，并根据触发器和视口空间向上或向下定位。弹层只让外层分组列表承担滚动，分组容器使用不创建滚动祖先的裁切方式，因此组标题可以相对外层列表吸顶，并在下一组到达时自然上推替换。当前 slot 与其他 slot 的占用标记由现有 `terminalSlots` 投影生成；自由排列继续复用 slot 替换逻辑，分组排列选择占用项时进入二级操作面板，由用户明确决定跳转或交换，`Esc` 或返回按钮可回到候选列表。搜索框按会话名或分组名过滤该投影：搜索期间匹配分组自动保持展开且暂时禁用折叠；分组命中保留组内全部会话，会话命中只保留对应项，清空搜索后恢复各分组持久化的折叠状态。
...
- 图片消息接口：`POST /api/agent-sessions/:id/image-message` 使用 `multipart/form-data` 接收一张图片和说明文字，同一入口服务 Codex 与 Claude。浏览器只提交 Kanban session ID，不提交主机、线程 ID、执行路径或临时路径；后端从 registry 读取可信会话元数据。Codex 沿用原链路：本地 tmux 复用完整记录的活动 pane 定位逻辑得到精确 thread，远端会话使用已登记的 Codex session ID，优先以参数数组执行 `codex queue --thread <id> --message <text> -i <path>`，CLI 拒绝图片附件时改投带绝对路径的文字消息并延迟最多 24 小时清理。Claude 只服务本机会话：先按 tmux pane 进程命令行里的 `--resume` 或 `--session-id` 精确定位，没有明确参数时仅当 `~/.claude/projects` 中工作目录唯一匹配一份记录才发送，无法唯一确定就拒绝且不猜测；图片以 base64 经 `claude -p --resume <session-id> --input-format stream-json --output-format stream-json` 的标准输入投进原会话，投递完成或失败都立即删除临时文件，远端 Claude 明确返回暂不支持。接口将单张图片限制为 10 MB，并按文件头只接受 PNG、JPEG、WebP。电脑端终端通过 xterm 自定义按键处理器把 `Ctrl/Cmd+V` 交还浏览器：浏览器捕获阶段同时从 `DataTransfer.files` 和 `DataTransfer.items` 提取图片并打开确认窗口，文本剪贴板则继续由 xterm 的原生 paste 事件发送。附件不写入项目目录、会话快照或完整记录接口。第一版只接入电脑端聚焦页，粘贴图片和文件选择共用同一预览/确认窗口，并提供显式取消入口。
...
- Claude 完整记录复用 transcript HTTP 路由，但独立解析本机或 SSH 的 `~/.claude/projects` JSONL；会话 ID、Claude tmux pane 或校验后的工作目录用于定位记录，字节游标负责分页。完成通知也保留独立 transcript UUID，使网页记录页和飞书只读查看/导出都能读取同一 Claude 历史；Claude UUID 不进入 Codex 回复路由。
...
- 当前会话页面采用单会话全屏终端：顶部显示桌面入口、当前会话状态和单实例页面内会话选择器，“完整记录 / 变更 / 文件”在同一行操作组；“文件”就地切换到当前会话的文件系统并提供“返回终端”，中间终端仅在返回后重新展示。会话列表不使用浏览器原生选择层，实时快照更新时不会重复堆叠，支持点外部或 Escape 关闭。快捷键条和发送/粘贴输入区组成连续、不可压缩的底部控制区；输入框聚焦时页面依据 `VisualViewport` 同步软键盘上方的可用高度与顶部偏移，并临时移除底部主导航占位和安全区空白，使控制区紧贴软键盘。
...
- “项目/文件”按主机与项目目录聚合会话，并提供适配触屏的文件系统入口。本地项目直接读取工作目录，SSH 项目沿用会话的远端连接信息；用户可进入目录、搜索当前目录、切换隐藏文件、在独立纵向滚动区预览文本或图片并复制路径。路径工具栏的“新建”打开触屏底部菜单，菜单内的原生输入框会从点击手势同步挂载并聚焦，以可靠唤起手机输入法；用户输入单层名称后可选择创建空文件或文件夹。前端拒绝空名称、`.`、`..`、空字节和路径分隔符，再复用 `useFileBrowser` 的本地/SFTP 创建与刷新链路。文件预览默认只保留一行紧凑标题栏，路径、复制路径、查看方式和分段导航收进可折叠的“文件选项”，展开区域自身有高度上限和滚动边界，避免挤占正文。Markdown 文件复用电脑端按需加载的安全 GFM/KaTeX 渲染组件，默认显示渲染结果，也可通过触屏友好的“渲染 / 源码”控件查看原始 Markdown；窄屏下图片自适应、表格和代码块可横向滚动。大型 UTF-8 文件不再固定截取开头：前端通过 `/api/fs/preview` 的 `offset` 继续请求 64 KiB 窗口，用户可前后切换；切换查看方式不会复制内容，翻段后也会替换而非追加内容，因此浏览器内存与文件总大小无关。文件条目长按 600ms 弹出底部操作菜单，滑动超过 12px 会取消，避免滚动误触；菜单可进入/预览、下载、重命名、删除和复制路径，删除继续使用现有本地/SFTP 安全校验并要求确认。
...
- 文件预览，文本文件可编辑保存。电脑端已加载的文本、图片和 Markdown 文件均可从预览标题栏进入浏览器级全屏阅读；全屏层复用当前渲染模式、目录、分段窗口和编辑状态，可通过标题栏按钮或 `Esc` 退出并把焦点还给原入口。普通图片不再使用最多 256 KiB 的分段预览前缀，而是由桌面与手机共享的图片组件请求 `POST /api/fs/image` 完整流：本机直接读取，远端复用已登记的 SSH/SFTP，前端创建并按生命周期释放 Blob URL，在预览容器内按比例完整缩放。接口只允许已知图片 MIME、设置 `nosniff`、拒绝目录与非图片，并将单张资源限制在 16 MiB。手机端仍使用既有的页面内文件预览。
- Markdown 文件默认渲染 GFM 和 KaTeX 数学公式；支持 `$...$` / `\(...\)` 行内公式与 `$$...$$` / `\[...\]` 块级公式。反斜线分隔符只在普通 Markdown 文本中规范化，行内代码、fenced code 和缩进代码保持原样。单击在文件列表下方显示紧凑预览；双击会让左侧文件面板进入全高内联预览并隐藏列表，右侧终端始终保持可见。“返回文件列表”恢复列表和原预览高度；全高预览从 Markdown 标题生成可折叠目录和稳定行锚点，点击目录只滚动文件正文。实时分屏默认按可滚动范围双向同步源码与渲染区，并提供独立滚动开关。电脑端通过既有 `/api/fs/preview` 偏移协议连续合并最多约 1 MiB 为当前窗口，超大文件前后翻段时替换正文而不累积；分段窗口为只读，只有从文件头完整载入且无下一段时才允许编辑保存。Markdown 图片由共享渲染组件区分外部 URL 与文件资源：HTTP(S) 保持浏览器原生懒加载；本地和 SSH 图片使用文档路径、项目根及已登记 SSH 目标请求 `POST /api/fs/markdown-image`，服务端解析 URL 编码和文档相对/项目根相对路径，并在真实路径层再次拒绝越出根目录的符号链接。接口仅流式返回已知图片 MIME，设置 `nosniff`，单张限制 16 MiB；前端只在图片接近视口时创建 Blob URL，并在预览卸载、文档切换或重试时撤销旧 URL。原始 HTML 不注入页面，桌面与手机端共用该资源链路。
...
- `FeishuReplyEventListener`：按事件能力启动或停止 `lark-cli` 飞书事件长连接；消息回复仍受独立回复开关控制，任务总览在回复控制关闭时保留只读菜单能力，并对 NDJSON 事件做有界、串行消费和退避重连。
- `FeishuReplyCommandService`：执行私聊发送者、回复关系、文字/图片消息和会话可控状态的白名单校验；精确通知绑定会验证原始 Codex thread 仍位于对应 tmux session，再投递到该 thread，而非跟随当前活动 pane。
...
- `TopBar.tsx`：分组顶栏、统一设置面板、操作提示、资源调节、飞书通知/回复开关、主入口和折叠。
- `feishu-notification-settings-service.ts`：解析脱敏的本地飞书配置状态，原子持久化两个独立开关，并拒绝在目标缺失、冲突或非私聊时开启对应能力。
...
飞书通知、回复续跑启用状态和 `kanban` 发送模式单独写入 `.dev-runtime/feishu-notification-settings.json`；该文件不包含接收者 ID 或机器人凭证，并与 `.env` 一样保持在 Git 之外。后端启动时保留旧通知开关值并迁移发送模式，新增回复开关默认关闭。发送成功的飞书 `message_id` 与看板会话绑定、以及已处理事件 ID 写入 `.dev-runtime/feishu-reply-bindings.json`，默认保留 30 天、限制总量且不保存通知正文或用户回复。
