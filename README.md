...
| 飞书通知与续跑 | 所有已登记看板任务结束后可通过本地 `lark-cli` 机器人发送 Card 2.0 私聊/群聊完成卡片；私聊用户还可回复某张卡片，把文字或图片安全送回对应的在线 Codex 终端 |
...
>
> Kanban 后端会观察已登记会话的真正完成点，再由本机 `lark-cli` 机器人向指定用户或群聊发送 Card 2.0 完成卡片。按钮开启前已经运行的 Codex 以及之后启动的其他 Agent 会话都能覆盖，不依赖页面保持打开；Codex 通知优先携带对话最后一条完整回复，长正文自动分成多张卡片。使用私聊目标时，还可单独开启“飞书回复继续执行”，回复某张卡片即可把文字或单张图片送回它绑定的在线 Codex 终端。个人接收者 ID、登录态、回复绑定和凭证继续只保存在本机。配置与边界见 [Kanban 任务完成飞书通知与回复续跑](docs/codex-feishu-notifications.md)。
...
- `2026-09-18` — 同一个 tmux 会话分屏运行多个 Codex 时，Kanban 会逐 pane 定位真实 Codex thread，并按 thread 独立建立完成基线、去重和发送飞书卡片；回复卡片、查看完整记录及文件按钮继续绑定原始分屏，不会因切换活动 pane 而串到另一个 Codex。

- `2026-09-18` — 飞书私聊完成卡片识别 Codex 明确输出的本机项目 Markdown 文件/图片链接，将无效的本地 URL 改为正常路径，并为最多 5 个安全文件增加小号查看按钮；安全的 PNG、JPEG、WebP、GIF 会上传为飞书卡片原生图片直接展示并支持点按放大，其他文件点击后无需访问内网网页即可预览或下载。

- `2026-09-18` — 飞书私聊回复支持把单张 PNG、JPEG 或 WebP 图片送入对应的本机/SSH Codex；仅接受已绑定通知或回复链，下载后校验真实格式与 10 MB 上限，成功入队后才继承绑定和去重。
...
- `2026-09-09` — 飞书新增「任务总览」菜单：默认可只读查看会话状态统计、最近输出摘要、分页和刷新；开启回复控制后可选择本页 Codex 发送指令。复用现有事件订阅，只需新增菜单 Key `kanban_task_overview`，见 [配置说明](docs/feishu-codex-control-panel.md)。

- `2026-09-09` — 飞书机器人支持固定菜单「Codex 对话」，通过独立面板选择会话并发送指令；保留原通知卡片回复续聊。需配置应用菜单和事件，见 [配置说明](docs/feishu-codex-control-panel.md)。

- `2026-09-07` — 完整记录支持逐条折叠/展开消息，长 Goal 输入可收成一行摘要；同一会话刷新和切换全屏保留折叠状态，覆盖电脑及手机端。
...
- **资源调节**：完整记录缓存窗口、VS Code iframe 缓存、手动释放缓存和资源诊断。
- **飞书通知**：分别控制所有已登记看板任务的完成提醒，以及私聊回复继续执行；已配置并开启飞书能力时，任务总览菜单仍可只读查看 session 状态和摘要。开启前已经运行的会话后续完成时同样发送，目标必须先在本地 `.env` 配置，页面不会读取或展示具体接收者 ID。回复控制默认关闭，群聊目标不能开启。

看板和聚焦侧栏固定使用轻量文本预览，不为每张卡片创建 xterm 和终端 WebSocket；只有进入聚焦或手机“当前会话”后才按需加载真实终端。资源调节中的记录缓存选项控制完整记录最多保留 90 或 300 条已加载消息，不会增加终端连接。
...
- [ ] **飞书回复续跑（可选）**：仅使用私聊 `FEISHU_NOTIFY_USER_ID`；应用已开通 `im:message.p2p_msg:readonly`，若要回复图片还需开通 `im:message:readonly`，并在飞书开放平台订阅 `im.message.receive_v1` 长连接事件。群聊目标只能接收通知，不能启用回复控制。
- [ ] **飞书开关**：启动后进入“设置 → 飞书通知”，确认目标类型，再分别按需开启“任务完成通知”和默认关闭的“飞书回复继续执行”。
- [ ] **飞书会话菜单（可选）**：已启用机器人菜单与卡片回调；「任务总览」只读列表要求飞书通知已配置并开启，Codex 指令、通知回复、完整记录和文件工作区还需显式开启“飞书回复控制”。发送文件需开通 `im:resource`。从菜单选择会话后点击“查看所选会话 / 记录 / 文件”，可查看记录、下载文件及确认保存小文本。文件读写暂限本机 Linux 项目，内容会传入飞书，详见 [使用与安全边界](docs/feishu-session-workspace.md)。
...
实际端口以 `.env` 和脚本输出为准。手机和电脑统一使用 HTTPS 主入口；终端默认连接同源 WSS，如果浏览器始终无法完成 WSS 握手，会自动切换到同一 `8484` HTTPS 源上的流式输出，无需更换地址或暴露后端端口。
...
- WSS 主通道支持文本 stdin、resize 和 tmux 鼠标等 binary 消息；WSS 从未建立时自动回退到同源 HTTPS 流读取输出，并复用 REST stdin/resize 写入。
- 实时连接异常后以 250ms～5s 有界退避重连；WSS 长期停在 `CONNECTING` 时主动回收并切换 HTTPS 流，已经成功连接过的 WSS 普通断线仍优先恢复 WSS。
...
- 看板和聚焦侧栏固定使用轻量预览，不为非活跃卡片挂载 xterm。
- 新打开的桌面终端最多回放最近 512 KiB，手机终端最多回放最近 256 KiB；更早的 Codex 内容通过“完整记录”读取。
- 会话数量超过阈值时按视口虚拟化卡片。
...
3. 在“资源调节”中调整完整记录缓存、VS Code 缓存模式或打开资源诊断。
